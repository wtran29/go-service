...
# Database Access
# dblab --host 0.0.0.0 --user postgres --db postgres --pass tango1337 --ssl disable --port 5432 --driver postgres
...
metrics:
	docker build \
		-f containers/docker/dockerfile.metrics \
		-t metrics:$(VERSION) \
		--build-arg BUILD_REF=$(VERSION) \
		--build-arg BUILD_DATE=`date -u +"%Y-%m-%dT%H:%M:%SZ"` \
		.
...
KIND_CLUSTER := services-starter-cluster

dev-up:
	kind create cluster \
		--image kindest/node:v1.25.3@sha256:f52781bc0d7a19fb6c405c2af83abfeb311f130707a0e219175677e366cc45d1 \
		--name $(KIND_CLUSTER) \
		--config containers/k8s/dev/kind-config.yaml
...
dev-load:
	cd containers/k8s/dev/sales; kustomize edit set image sales-api-image=sales-api:$(VERSION)
	kind load docker-image sales-api:$(VERSION) --name $(KIND_CLUSTER)

	cd containers/k8s/dev/sales; kustomize edit set image metrics-image=metrics:$(VERSION)
	kind load docker-image metrics:$(VERSION) --name $(KIND_CLUSTER)

dev-apply:
	kustomize build containers/k8s/dev/vault | kubectl apply -f -

	kustomize build containers/k8s/dev/database | kubectl apply -f -
	kubectl wait --timeout=120s --namespace=sales-system --for=condition=Available deployment/database
	
	kustomize build containers/k8s/dev/zipkin | kubectl apply -f -
	kubectl wait --timeout=120s --namespace=sales-system --for=condition=Available deployment/zipkin
	
	kustomize build containers/k8s/dev/sales | kubectl apply -f -
...
dev-services-delete:
	kustomize build containers/k8s/dev/sales | kubectl delete -f -
	kustomize build containers/k8s/dev/zipkin | kubectl delete -f -
	kustomize build containers/k8s/dev/database | kubectl delete -f -
