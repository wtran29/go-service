...
service:
	docker build \
		-f zContainers/docker/dockerfile.service \
		-t $(SERVICE_IMAGE) \
...
metrics:
	docker build \
		-f zContainers/docker/dockerfile.metrics \
		-t $(METRICS_IMAGE) \
		--build-arg BUILD_REF=$(VERSION) \
		--build-arg BUILD_DATE=`date -u +"%Y-%m-%dT%H:%M:%SZ"` \
		.
...
dev-up-local:
	kind create cluster \
		--image $(KIND) \
		--name $(KIND_CLUSTER) \
		--config zContainers/k8s/dev/kind-config.yaml
...
dev-load:
	...
	cd zContainers/k8s/dev/sales; kustomize edit set image metrics-image=$(METRICS_IMAGE)
	kind load docker-image $(METRICS_IMAGE) --name $(KIND_CLUSTER)

dev-apply:
	kustomize build zContainers/k8s/dev/vault | kubectl apply -f -
